...
public class DanielBlue extends LinearOpMode{
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("Carriage Position: ", Carriage.getPosition());
    ...
    public void defaultMode() {
        ...
        if(gamepad1.right_stick_button && rightCooldown()) {
            double currentPos = Door.getPosition();
            Door.setPosition(currentPos + 0.05);
        }

        if(gamepad1.left_stick_button && leftCooldown()) {
            double currentPos = Door.getPosition();
            Door.setPosition(currentPos - 0.05);
        }

        if(gamepad1.dpad_up && upCooldown()) {
            double currentPos = Carriage.getPosition();
    ...
    public boolean upCooldown() {
        if(getRuntime() - upStartTime > .1) { //Must wait 250 milliseconds before input can be used again
    ...
    public boolean downCooldown() {
        if(getRuntime() - downStartTime > .1) { //Must wait 250 milliseconds before input can be used again
    ...
    public boolean rightCooldown() {
        if(getRuntime() - rightStartTime > .1) { //Must wait 250 milliseconds before input can be used again
    ...
    public boolean leftCooldown() {
        if(getRuntime() - leftStartTime > .1) { //Must wait 250 milliseconds before input can be used again
